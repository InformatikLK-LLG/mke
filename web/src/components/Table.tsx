...
// interface TableType<Type> { (tableHeaders: Array<String>, rows: Array<Type>): JSX.Element; }

import { Key } from "react";

// export default function Table<Type>({ tableHeaders, rows }: TableProps<Type>) {
...
export default function Table<T extends SimplestItem>({ tableHeaders, rows }: TableProps<T>) {
    ...
    return objectValues(value).map((nestedValue) => {
      return isPrimitive(nestedValue) ? (
        <td key={`${row.id}.${nestedValue}`}>
          {nestedValue}
          {console.log(`nested value of ${row.id}: ${nestedValue} with key ${row.id}.${nestedValue}`)}
        </td>
  ...
  function renderValue(row: T, value: T[keyof T]) {
    return isPrimitive(value) ? (
      <td key={`${row.id}.${value}`}>
        {value}
        {console.log(`primitive value of ${row.id}: ${value} with key ${row.id}.${value}`)}
      </td>
    ) : typeof value === "object" ? (
            ...
            objectValues(header).map((nestedHeader) => {
              return typeof nestedHeader === "string" ? (
                <th key={`${nestedHeader}`}>
                  ...
                  {console.log(`nestedHeader ${nestedHeader} with key ${nestedHeader}`)}
                </th>
              ) : (
                <th>empty</th>
              );
